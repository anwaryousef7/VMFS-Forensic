...
class VMFSGui(QtWidgets.QMainWindow):
    ...
    def _fill_tree_level(self, parent, dir_path):
        # ── FIX: use follow_symlinks=True so VMFS6 symlinked dirs/files
        #         are treated correctly.  Wrap per-entry to survive broken
        #         symlinks without aborting the whole directory listing.
        try:
            raw_entries = list(os.scandir(dir_path))
        except PermissionError:
            ...
            err = QtWidgets.QTreeWidgetItem(
                [f"[Error: {exc}]", "", "", dir_path])
            err.setForeground(0, QtGui.QColor("#ff8844"))
            parent.addChild(err)
        ...
        def _safe_is_dir(e):
            try:
                return e.is_dir(follow_symlinks=True)   # FIX: follow symlinks
            except OSError:
                return False

        try:
            entries = sorted(raw_entries,
                             key=lambda e: (not _safe_is_dir(e),
                                            e.name.lower()))
        except Exception:
            entries = raw_entries   # fallback: unsorted if key fails

        for entry in entries:
            try:
                is_dir = entry.is_dir(follow_symlinks=True)   # FIX
            except OSError:
                is_dir = False
            ...
            if is_dir:
                ...
                label = (entry.name + " →") if is_link else entry.name
                child = QtWidgets.QTreeWidgetItem(
                    [label, "", "DIR", entry.path])
                child.setIcon(0, icon)
                if is_link:
                    child.setForeground(0, QtGui.QColor("#44aaff"))
                child.addChild(QtWidgets.QTreeWidgetItem(["...", "", "", ""]))
            else:
                try:
                    sz       = entry.stat(follow_symlinks=True).st_size  # FIX
    ...
    def _export_selected(self):
        ...
        for src in paths:
            if os.path.isfile(src):
                ...
                for dirpath, _, filenames in os.walk(src, followlinks=True):  # FIX
    ...
    def _save_tree_report(self):
        ...
        def _safe_is_dir_w(e):
            try:
                return e.is_dir(follow_symlinks=True)   # FIX
            except OSError:
                return False
        ...
        def walk(dirpath, prefix=""):
            ...
            try:
                entries = sorted(raw,
                                 key=lambda e: (not _safe_is_dir_w(e),
                                                e.name.lower()))
            except Exception:
                entries = raw
            ...
            for i, entry in enumerate(entries):
                ...
                try:
                    ...
                    is_dir = False
                sym_mark = " →" if entry.is_symlink() else ""
                if is_dir:
                    total_dirs[0] += 1
                    lines.append(
                        f"{prefix}{connector}{entry.name}{sym_mark}/")
