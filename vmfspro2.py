...
class ExportProgressDialog(QtWidgets.QDialog):
    ...
    def __init__(self, title="Exporting…", parent=None):
        ...
        self.bar.setStyleSheet("QProgressBar{height:30px;border-radius:5px;font-size:13px;} QProgressBar::chunk{background:#1a7a1a;border-radius:5px;}")
        lay.addWidget(self.bar)

        info_row = QtWidgets.QHBoxLayout()
        self.speed_label = QtWidgets.QLabel("Speed: --")
        self.speed_label.setStyleSheet("color:#44ff88;font-weight:bold;font-size:13px;")
        ...
        self.hash_label.setStyleSheet("color:#ffcc44; font-weight:bold; font-size:11px;")
        lay.addWidget(self.hash_label)

        b_cancel = QtWidgets.QPushButton("Cancel")
        b_cancel.setStyleSheet("background:#7a1a1a;color:white;font-weight:bold;padding:5px;")
        b_cancel.clicked.connect(self.cancel_requested.emit)
        lay.addWidget(b_cancel)

    def update_progress(self, pct: int, filename: str = "", speed: str = "", copied_str: str = "", h_md5="", h_sha256=""):
        self.bar.setValue(pct)
        if filename:
            name = filename[-60:] if len(filename) > 60 else filename
            self.file_label.setText(f"Copying:  {name}")
        if speed: self.speed_label.setText(f"Speed: {speed}")
        if copied_str: self.copied_label.setText(copied_str)
        if h_md5: self.hash_label.setText(f"MD5: {h_md5} | SHA256: {h_sha256[:20]}...")
...
class VMFSGui(QtWidgets.QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.tab_intel     = QtWidgets.QWidget()
        ...
        self.speed_label.setStyleSheet("color:#44ff88;font-size:13px;font-weight:bold;")
    ...
    def _build_tab_mount(self):
        ...
        self.mount_dev_override.setPlaceholderText("Leave empty to use table selection  or  /dev/sdc1")
        ...
        self.mount_sudo_chk = QtWidgets.QCheckBox("Use sudo  (required if not root)")
        self.mount_sudo_chk.setChecked(True)
        self.allow_other_chk = QtWidgets.QCheckBox("Add -o allow_other  (fixes permission denied)")
        self.allow_other_chk.setChecked(True)
        self.nonempty_chk = QtWidgets.QCheckBox("Add -o nonempty  (fixes mountpoint is not empty)")
        ...
        fuse_note = QtWidgets.QLabel("If allow_other fails, run once:\n   echo 'user_allow_other' | sudo tee -a /etc/fuse.conf")
        fuse_note.setStyleSheet("color:#aaddff;background:#1a2a3a;padding:6px;border-radius:4px;")
        lay.addWidget(fuse_note)

        btn_row = QtWidgets.QHBoxLayout()
        b_mount  = QtWidgets.QPushButton("Mount VMFS6")
        b_umount = QtWidgets.QPushButton("Unmount")
        b_mount.setStyleSheet("background:#1a7a1a;color:white;font-weight:bold;padding:6px;")
        b_umount.setStyleSheet("background:#7a1a1a;color:white;font-weight:bold;padding:6px;")
    ...
    def _build_tab_dd(self):
        ...
        self.dd_dev_override.setPlaceholderText("Leave empty or type e.g.  /dev/sdc1")
        ...
        self.dd_output_edit.setPlaceholderText("/home/user/vmfs6.dd  or  /mnt/evidence/image.e01")
        ...
        speed_grp  = QtWidgets.QGroupBox("Forensic Tool & Options")
        speed_form = QtWidgets.QFormLayout(speed_grp)
        self.dd_tool_combo = QtWidgets.QComboBox()
        self.dd_tool_combo.addItems([
            "dd (Fast Raw Image)", 
            "dc3dd (Hashed Raw Image - MD5/SHA256)", 
            "ewfacquire (E01 Compressed Forensic Image)"
        ])
        self.dd_bs_combo = QtWidgets.QComboBox()
        self.dd_bs_combo.addItems(["1M", "4M", "8M", "16M", "64M", "128M"])
        self.dd_bs_combo.setCurrentText("4M")
        
        self.dd_direct_in_chk = QtWidgets.QCheckBox("iflag=direct (bypass read cache - dd only)")
        self.dd_direct_in_chk.setChecked(True)
        self.dd_direct_out_chk = QtWidgets.QCheckBox("oflag=direct (bypass write cache - dd only)")
        self.dd_direct_out_chk.setChecked(True)
        self.dd_noerror_chk = QtWidgets.QCheckBox("conv=noerror,sync (continue on bad sectors - dd only)")
        self.dd_noerror_chk.setChecked(True)
        self.dd_sudo_chk = QtWidgets.QCheckBox("Use sudo")
        self.dd_sudo_chk.setChecked(True)
        
        speed_form.addRow("Tool:", self.dd_tool_combo)
        ...
        b_dd.setStyleSheet("background:#1a4a8a;color:white;font-weight:bold;padding:10px;font-size:14px;")
    ...
    def _build_tab_browse(self):
        ...
        b_export_sel = QtWidgets.QPushButton("⬇ Export & Hash Selected")
        b_export_sel.setStyleSheet("background:#1a5a1a;color:white;font-weight:bold;padding:7px;")
        b_export_sel.clicked.connect(self._export_selected)

        b_tree_report = QtWidgets.QPushButton("📄 Tree Report (.txt)")
        b_tree_report.setStyleSheet("background:#3a3a1a;color:white;font-weight:bold;padding:7px;")
        b_tree_report.clicked.connect(self._save_tree_report)
        
        b_timeline = QtWidgets.QPushButton("🕒 MAC Timeline (.csv)")
        b_timeline.setStyleSheet("background:#5a3a1a;color:white;font-weight:bold;padding:7px;")
        b_timeline.clicked.connect(self._generate_timeline)

        b_hex = QtWidgets.QPushButton("🔬 Hex Viewer")
        b_hex.setStyleSheet("background:#1a1a5a;color:white;font-weight:bold;padding:7px;")
    ...
    def _build_tab_forensics(self):
        ...
        self.carve_src.setPlaceholderText("Raw device /dev/sdX or .dd file")
        self.carve_out = QtWidgets.QLineEdit()
        self.carve_out.setPlaceholderText("/mnt/recovery_output")
        b_carve = QtWidgets.QPushButton("Start Carving")
        b_carve.clicked.connect(self._start_carving)
        form_carve.addRow("Target:", self.carve_src)
        form_carve.addRow("Output Dir:", self.carve_out)
        form_carve.addRow("", b_carve)
        ...
        self.yara_rule.setPlaceholderText("/path/to/ransomware.yar")
        self.yara_target = QtWidgets.QLineEdit()
        self.yara_target.setPlaceholderText("Directory or VMDK file to scan")
        b_yara = QtWidgets.QPushButton("Run YARA Scan")
        b_yara.clicked.connect(self._start_yara)
        form_yara.addRow("YARA Ruleset (.yar):", self.yara_rule)
        form_yara.addRow("Target to Scan:", self.yara_target)
        form_yara.addRow("", b_yara)
    ...
    def _build_tab_intel(self):
        ...
        b_scan_intel.setStyleSheet("background:#7a1a1a;color:white;font-weight:bold;padding:10px;font-size:14px;")
        ...
        self.vmx_table.horizontalHeader().setSectionResizeMode(QtWidgets.QHeaderView.Stretch)
        ...
        self.intel_log.setStyleSheet("background:#0f151f;color:#88ccff;font-family:Courier New, monospace;")
    ...
    def _populate_sysblock(self):
        ...
        for name in devs:
            ...
            self._add_device_row(f"/dev/{name}", size, "disk", "", "", "possible VMFS", QtGui.QColor("#ffcc44"))
            try:
                for entry in sorted(os.listdir(f"/sys/block/{name}")):
                    if entry.startswith(name):
                        ps = self._sysblock_size(name, entry)
                        self._add_device_row(f"/dev/{entry}", ps, "part", "", "", "possible VMFS", QtGui.QColor("#ffcc44"))
            except Exception: pass
        ...
        for row in range(self.device_table.rowCount()):
            ...
            if (fs.text().strip() == "" and tp.text().strip() in ("disk", "part", "Physical") and (not mnt or mnt.text().strip() in ("", "-"))):
                ...
                for c in range(self.device_table.columnCount()):
                    ...
                    if it: it.setForeground(QtGui.QColor("#ffcc44"))
                hits += 1
    ...
    def _umount_vmfs(self):
        ...
        def task(worker: WorkerThread):
            ...
            if subprocess.run(["sudo", "umount", mp], capture_output=True).returncode != 0:
    ...
    def _start_dd(self):
        ...
        def task(worker: WorkerThread):
            ...
            if "ewfacquire" in tool:
                ...
                # Regular dd with advanced options
            ...
            for line in iter(proc.stdout.readline, ""):
                ...
                if line.strip(): worker.log_message.emit(line.strip()[:150]) # Prevent massive lines
    ...
    def _load_vmfs_tree(self):
        ...
        root_item.setIcon(0, self.style().standardIcon(QtWidgets.QStyle.SP_DriveHDIcon))
    ...
    def _tree_context_menu(self, pos):
        ...
        if not items: return
        menu   = QtWidgets.QMenu(self)
        a_exp  = menu.addAction("⬇ Export & Hash Selected")
    ...
    def _export_selected(self):
        ...
        dest_dir = QtWidgets.QFileDialog.getExistingDirectory(self, "Select destination folder for export")
        ...
        dlg = ExportProgressDialog(f"Exporting & Hashing {len(copy_plan)} file(s)…", parent=self)
        stop_flag = [False]
            ...
            try:
                ...
                with open(sf, "rb") as fin, open(df, "wb") as fout:
                    while True:
                        ...
                        pct = min(int(copied * 100 / total_bytes), 99) if total_bytes else 0
                        spd = f"{copied/(time.time()-t0)/1024**2:.1f} MB/s" if (time.time()-t0) > 0.5 else "--"
                        dlg.update_progress(pct, os.path.basename(sf), spd, f"{self._fmt_size(copied)} / {self._fmt_size(total_bytes)}", md5.hexdigest(), sha256.hexdigest())
                        QtWidgets.QApplication.processEvents()
                self._log(f"Exported: {sf} -> MD5: {md5.hexdigest()}")
            except Exception as e: self._log(f"Error copying {sf}: {e}")

        dlg.update_progress(100, "Done ✓", "", self._fmt_size(copied))
        QtWidgets.QApplication.processEvents()
        dlg.close()
        self._log(f"Export complete. Time: {time.time()-t0:.1f}s")
                ...
                for fname in filenames:
                    ...
                    if fname.endswith(".vmx"):
                        ...
                        try:
                            ...
                            QtCore.QMetaObject.invokeMethod(self, "_add_vmx_row", QtCore.Qt.QueuedConnection, QtCore.Q_ARG(str, vm_name), QtCore.Q_ARG(str, guest_os), QtCore.Q_ARG(str, mac_addr), QtCore.Q_ARG(str, filepath))
                        except: pass

                    # 2. Snapshot Chain Analyzer
    ...
    def _log(self, text: str):
        ...
        if "Thin Prov" in text or "Volume Metadata" in text or "MAC" in text or "Heartbeat" in text:
