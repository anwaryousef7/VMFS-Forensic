...
class VMFSGui(QtWidgets.QMainWindow):
    ...
    def _resolve_source(self, cb, override_edit):
        ...
        if "device" in cb.currentText().lower():
            ...
            if not dev:
                raise RuntimeError(
                    "No device selected.\n"
                    "Click a row in Sources tab, or type a path in the override field.")
            return dev
        ...
        def task(worker: WorkerThread):
            ...
            if rc not in (0, None):
                raise RuntimeError(
                    ...
                    "Tips:\n"
                    "  - Use the partition e.g. /dev/sdc1 not /dev/sdc\n"
                    "  - Check: sudo dmesg | tail -20\n"
                    "  - Make sure sudo checkbox is enabled")
    ...
    def _load_vmfs_tree(self):
        ...
        if not root or not os.path.isdir(root):
            QtWidgets.QMessageBox.warning(
                ...
                "Mount it first in the Mount tab.")
            return
    ...
    def _gen_wsl_cmds(self):
        ...
        lines += [
            "",
            "# Inside WSL: refresh Sources tab – disk appears as /dev/sdX",
        ]
